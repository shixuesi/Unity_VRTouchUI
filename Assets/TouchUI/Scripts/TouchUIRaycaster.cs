...
namespace TouchUI
{
    ...
    public class TouchUIRaycaster : GraphicRaycaster, IPointerEnterHandler
    {

        protected TouchUIRaycaster()
        { }
        ...
        private void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList, Ray ray, bool checkForBlocking)
        {
            ...
            for (var index = 0; index < m_RaycastResults.Count; index++)
            {
                ...
                if (appendGraphic)
                {
                    ...
                    TouchLimit lim = go.GetComponent<TouchLimit>();//获取距离限制脚本
                    if (lim)
                    {
                        if (!lim.isTouched(distance))//如果大于触发距离
                            continue;
                    }
                    else
                        continue;
        ...
        {
            TouchUIPointerEventData rayPointerEventData = eventData as TouchUIPointerEventData;
            if (rayPointerEventData != null)
            {
                Raycast(eventData, resultAppendList, new Ray(rayPointerEventData.pointerPos,canvas.transform.forward), true);
            }
        }
        /// <summary>
        ...
        {
            TouchUIInputModule inputModule = EventSystem.current.currentInputModule as TouchUIInputModule;
            return inputModule && inputModule.activeGraphicRaycaster == this;
        }

        public void OnPointerEnter(PointerEventData e)
        {
            PointerEventData ped = e as TouchUIPointerEventData;
            if (ped != null)
            {
